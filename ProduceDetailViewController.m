...
- (void)setNavigationAttributes
{
    ...
    self.navigationController.navigationBar.topItem.title = @"产品详细";
}
- (void)createView
{
...
- (void)viewDidLoad
{
    ...
    [self initData];
    [self createView];
    [self getProduceDetailData];
    
}

- (void)getProduceDetailData
{
    [self showWithLoding];
    
    listArray = [recordDB resultSet:PRODUCEDETAIL_TABLENAME Order:nil LimitCount:0];
    [recordDB deleteAtIndex:PRODUCEDETAIL_TABLENAME CloValue:0];
    if([listArray count] == 0)
    {
        GetObj *getO = [[GetObj alloc]init];
        getO.pid = self.pid;
        NSString *produceDetailURL = [urlStr returnURL:32 Obj:getO];
        
        
        [jsonParser parse:produceDetailURL withDelegate:self onComplete:@selector(connectionProduceDetailSuccess:) onErrorComplete:@selector(connectionError)];
    }
    else
    {
        [self initWebView];
...
- (void)connectionProduceDetailSuccess:(JsonParser *)jsonP
{
    NSArray *returnDic = [jsonP getItems];
    for (int i = 0 ; i<[returnDic count]; i++) {
       // NSArray *produceDetailArray = @[[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"id"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"title"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"content"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"catid"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"dateline"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"hits"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"price"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"xinghao"]];

        NSArray *produceDetailArray = [[NSArray alloc] initWithObjects:[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"id"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"title"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"content"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"catid"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"dateline"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"hits"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"price"],[(NSDictionary *)[returnDic objectAtIndex:i] objectForKey:@"xinghao"], nil];
        [recordDB insertAtTable:PRODUCEDETAIL_TABLENAME Clos:produceDetailArray];
    }
